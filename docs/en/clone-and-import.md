...
`git clone https://github.com/ByteLegendQuest/java-fix-infinite-loop`

A project directory named `java-fix-infinite-loop` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-infinite-loop` directory which is created just now.
